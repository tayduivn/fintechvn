...
export const del = (id) => {
  return (dispatch: (action: Action) => void, getState: () => GlobalState) => {
    ...
    return api.productDetail.del(id)
      .then(res => {
        if(res.error)  dispatch(fetchFailed(res.error));
        else dispatch(delFinished(id));
        return res;
      })
  }
}
...
export const updateById = (id, data) => { 
  return (dispatch: (action) => void) => {
    ...
    return api.productDetail.updateById(id, data)
      .then(obj => {
        if(obj.error)  dispatch(fetchFailed(obj.error));
        else dispatch(fetchFinished([obj.data]))
        return obj
      });
  }
}
