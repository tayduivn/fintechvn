...
import * as seatsActions from './actions';
...
class ListUser extends Component {
  ...
  componentDidMount(){
    let { seats, seatsActions, profile } = this.props;
  ...
  formSubmitData = (data) => {
    let { seatsActions, notification, profile} = this.props;
    let { idUpdate } = this.state;
    
    if(!idUpdate){
      data.insur_id = profile.info.agency.id;
      seatsActions.create(data)
        .then(res => {
  ...
  updateItemById = (id, data, titleS) => {
    let { seatsActions, notification} = this.props;

    seatsActions.updateById(id, data)
      .then(res => {
        if(res.error) return Promise.reject(res.error);
        if(!res.data) return Promise.reject({messagse: "unknown error"});
  ...
  render() {
    ...
    let { data, ordered, isWorking }    = seats;
...
let mapStateToProps = (state) => {
  let { seats } = state.categories;
  let { profile } = state;

  return { seats, profile };
};
