...
class PriceFast extends React.Component {
  ...
  render() {
    let { dataRequest, disabled, t, seatsPayload, ruleExtend } = this.props;
    ...
    return (
      <React.Fragment>
        ...
        {
          !!ruleExtend.ordered && !isEmpty(ruleExtend.ordered) && ruleExtend.ordered.map(e => {
            return (
              <div key={e} className="col-md-12">
                <div className="checkbox checkbox-info pull-left col-md-12 m-t-15">
                  ...
                  <label htmlFor={e} > {ruleExtend.data[e].name ? ruleExtend.data[e].name : ""} </label>
                </div>
              </div>
            )
          })
...
let mapStateToProps = (state) => {
  let { profile } = state;
  let { years, seatsPayload, ruleExtend } = state.categories;
  return { years, profile, seatsPayload, ruleExtend };
};
