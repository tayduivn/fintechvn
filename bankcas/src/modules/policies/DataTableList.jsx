...
class DataTableList extends Component {
  ...
  rowGetter = ({ index }: { index: number }): any => {
    ...
    startDay = convertDMY(startDay); //`${date.getDate()} - ${date.getMonth() + 1} - ${date.getFullYear()}`;

    endDay = convertDMY(endDay); //`${date.getDate()} - ${date.getMonth() + 1} - ${date.getFullYear()}`;
    ...
    return (
      <Dropdown icon='ellipsis horizontal' >
        <Dropdown.Menu>
          ...
          <Dropdown.Item onClick={this.handelClone(id)} text={t('policies:acClone')} icon="paper plane outline"/>
        </Dropdown.Menu>
      </Dropdown>);
  }
