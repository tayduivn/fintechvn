...
class Clone extends Component {
  ...
  handelSubmitProduct = () => {
    ...
    if(result.error.length === 0){
      let { profile, productDetailActions, match, productDetail } = this.props;
      let { id } = match.params;
  ...
  render() { 
    ...
    let panes = [
      { menuItem: t('requests:tabProduct'), render: () => (
        <Tab.Pane>
          <FormData action="clone" dataRequest={dataRequest} data={dataProduct} t={ t } />
        </Tab.Pane>)}
    ]
    return (
      <Form loading={productDetail.isWorking} method="post" id="formProduct" onSubmit={this.handelSubmitProduct}>
...
export default withNotification(translate('requests')(connect(mapStateToProps, mapDispatchToProps)(Clone)));