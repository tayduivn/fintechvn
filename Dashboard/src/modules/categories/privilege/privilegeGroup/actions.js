...
export const fetchAll = (filter?, skip?, limit?, where?) => {
  return (dispatch: (action: Action) =>void) => {
    ...
    return api.privilegeGroup.get(filter, skip, limit, where)
      .then(res => {
...
export const create = (data) => {
  return (dispatch: (action) => void) => {
    dispatch(fetchStarted());
    return api.privilegeGroup.create(data)
      .then(obj => {
        if(obj.error)
...
export const updateById = (id, data) => { 
  return (dispatch: (action) => void) => {
    dispatch(fetchStarted());
    return api.privilegeGroup.updateById(data, id)
      .then(obj => {
        if(!!obj.data)
          dispatch(fetchFinished([obj.data]))
...
export const deleteItem = (id) => {
  return (dispatch: (action: Action) => void, getState: () => GlobalState) => {
    dispatch(fetchStarted());
    return api.privilegeGroup.del(id)
      .then(res => {
